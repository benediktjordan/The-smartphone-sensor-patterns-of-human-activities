...
class Static_Periods:
    ...
    def compute_long_periods_of_static_updated(df_participant_all, threshold_expected, threshold_static_percentage, threshold_static, window_size_minutes, window_step_minutes, sensor_frequency):
        ...
        while end_time_window < end_time:
            ...
            # check if window contains at least "threshold_expected" % of expected values
            if len(df_participant_window) >= expected_values * threshold_expected:
                # calculate how many values are above threshold_static in column "double_values_2"
                number_of_values_below_threshold = len(df_participant_window[df_participant_window["double_values_2"] < threshold_static])

                # check if most frequent value is at least 95% of all values
                if number_of_values_below_threshold >= len(df_participant_window) * threshold_static_percentage:
                    # add to df_results
                    df_results.loc[len(df_results)] = [participant, start_time_window, end_time_window, end_time_window - start_time_window, len(df_participant_window), expected_values, len(df_participant_window) / expected_values, number_of_values_below_threshold, number_of_values_below_threshold / len(df_participant_window)]
                ...
                df_analytics.loc[len(df_analytics)] = [participant, start_time_window, end_time_window,
                                                     ...
                                                     number_of_values_below_threshold, number_of_values_below_threshold / len(df_participant_window)]

            else:
                # get analytics
                number_of_values_below_threshold = len(df_participant_window[df_participant_window["double_values_0"] < threshold_static])
                df_analytics.loc[len(df_analytics)] = [participant, start_time_window, end_time_window,
                                                     end_time_window - start_time_window, len(df_participant_window),
                                                     expected_values, len(df_participant_window) / expected_values,
                                                     number_of_values_below_threshold, number_of_values_below_threshold / len(df_participant_window)]
    ...
    def compute_long_periods_of_static(df_participant_all, threshold_expected, threshold_statis,
                                       window_size_minutes, window_step_minutes, sensor_frequency):
        # compute long periods of static
        df_results = pd.DataFrame(
            columns=["participant", "start_time", "end_time", "duration", "actual_values", "expected_values",
                     "percentage_expected", "static_values", "percentage_static"])
        df_analytics = pd.DataFrame(
            columns=["participant", "start_time", "end_time", "duration", "actual_values", "expected_values",
                     "percentage_expected", "static_values", "percentage_static"])

        # truncate the sensor_column to 3 decimals (dont round but cut): to decrease sensitivity of the sensor
        df_participant_all["double_values_0"] = df_participant_all["double_values_0"].apply(
        ...
        # calculate total number of windows
        total_number_of_windows = int((end_time - start_time).total_seconds() / 60 / window_step_minutes) - int(
            window_size_minutes / window_step_minutes)
        ...
        while end_time_window < end_time:
            ...
            # get value counts of double_values_0
            df_participant_window = df_participant_all[(df_participant_all["timestamp"] >= start_time_window) & (
                        df_participant_all["timestamp"] < end_time_window)]
            ...
            if len(df_participant_window) >= expected_values * threshold_expected:
                ...
                if most_occuring_sum >= len(df_participant_window) * threshold_statis:
                    # add to df_results
                    df_results.loc[len(df_results)] = [participant, start_time_window, end_time_window,
                                                       end_time_window - start_time_window,
                                                       len(df_participant_window), expected_values,
                                                       len(df_participant_window) / expected_values,
                                                       most_occuring_sum,
                                                       most_occuring_sum / len(df_participant_window)]
                    print("long period of static for start time window: " + str(
                        start_time_window) + " and end time window: " + str(end_time_window))

                # get analytics
                df_analytics.loc[len(df_analytics)] = [participant, start_time_window, end_time_window,
                                                       end_time_window - start_time_window,
                                                       len(df_participant_window),
                                                       expected_values,
                                                       len(df_participant_window) / expected_values,
                                                       most_occuring_sum,
                                                       most_occuring_sum / len(df_participant_window)]
        ...
        for index, row in df_results.iterrows():
            ...
            if index < df_length - 1:
                ...
                while df_results.loc[index_new + 1]["start_time"] < row["end_time"]:
                    ...
                    df_results.at[index, "end_time"] = df_results.loc[index_new + 1]["end_time"]
                    # add expected values, actual values and static values of next row to current row
                    df_results.at[index, "expected_values"] = df_results.loc[index_new + 1]["expected_values"] + \
                                                              df_results.loc[index]["expected_values"]
                    df_results.at[index, "actual_values"] = df_results.loc[index_new + 1]["actual_values"] + \
                                                            df_results.loc[index]["actual_values"]
                    df_results.at[index, "static_values"] = df_results.loc[index_new + 1]["static_values"] + \
                                                            df_results.loc[index]["static_values"]
                    # update row time
                    row["end_time"] = df_results.loc[index_new + 1]["end_time"]
